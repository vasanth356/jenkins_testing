...
class ClassificationMetrics:
    def __init__(self, package, average, metric_name=None, beta=None, pred_prob=None):
        ...
        if metric_name is None:
            ...
            self._auc(self.y_true, self.y_pred,self.average)
            ...
            self._logloss(self.y_true, self.pred_prob)
    ...
    def _run_all(self):
        ...
        self._recall(self.y_true, self.y_pred, self.average)
        self._f1_score(self.y_true, self.y_pred, self.average)
        self._fbeta_score(self.y_true, self.y_pred,self.beta, self.average)
        self._auc(self.y_true, self.y_pred,self.average)
        self._matthews_corrcoef(self.y_true, self.y_pred)
        self._hamming_loss(self.y_true, self.y_pred)
        self._logloss(self.y_true, self.pred_prob)
    ...
    @staticmethod
    def _fbeta_score(y_true, y_pred, beta, average):
        f_beta = ['F-Beta Score', metrics.fbeta_score(y_true, y_pred, beta=beta,average=average), '1']
        MetricList.append(f_beta)

    @staticmethod
    def _auc(y_true, y_pred, average):
        ...
        auc = ['AUC Score', metrics.roc_auc_score(y_true, y_pred,average=average, multi_class='ovr'), '1']
    ...
    @staticmethod
    def _logloss(y_true, pred_prob):
        try:
            log_loss = ['Log Loss', metrics.log_loss(y_true, pred_prob), '0']
            MetricList.append(log_loss)
        except:
            log_loss = ['Log Loss', 'None', '0']
            MetricList.append(log_loss)
